...
def get_atomic_facts_vector_index(db:Neo4jGraph, k_atomic_facts:int, embedding_question:List[float], embedding_model:str) -> List[Dict[str, str]]:
    """
    Retrieves AtomicFact that are most similar to the given question embedding, using the VectorIndex.

    Args:
    - db: The Neo4jGraph object for querying the graph database.
    - k_atomic_facts: The number of AtomicFacts to retrieve.
    - embedding_question: The embedding of the question.
    - embedding_model: The embedding model to use for similarity calculation.
    ...
    data = db.query(f"""
                    CALL db.index.vector.queryNodes('AtomicFact_{embedding_model}', {k_atomic_facts}, $input_embedding)
                    YIELD node as atomic_fact, score
                    MATCH (c:Chunk)-[:HAS_ATOMIC_FACT]->(atomic_fact)
                    RETURN atomic_fact.id as atomic_fact_id, atomic_fact.content AS text, c.id AS chunk_id 
                    ORDER BY score DESC
                    """, 
